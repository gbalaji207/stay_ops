...
class BookingSourceReportScreen extends StatelessWidget {
  const BookingSourceReportScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => ReportsCubit(ReportsRepository()),
      child: const _BookingSourceReportView(),
    );
  }
}

class _BookingSourceReportView extends StatefulWidget {
  const _BookingSourceReportView();

  @override
  State<_BookingSourceReportView> createState() =>
      _BookingSourceReportViewState();
}

class _BookingSourceReportViewState extends State<_BookingSourceReportView> {
  ...
  void _load() {
    ...
    context.read<ReportsCubit>().loadBookingSourceReport(
  ...
  Future<void> _exportCsv(BookingSourceReportLoaded state) async {
    ...
    final fname   = 'booking_source_report_$dateStr.csv';
    ...
    if (kIsWeb) {
      ...
      await Share.shareXFiles(
        ...
        subject: 'Booking Source Report $dateStr',
      );
    }
  }
        ...
        actions: [
          Builder(
            builder: (ctx) {
              ...
              final loaded =
                  st is BookingSourceReportLoaded && st.roomRows.isNotEmpty
                      ? st
                      : null;
              return IconButton(
                icon: Icon(
          ...
          Expanded(
            child: BlocBuilder<ReportsCubit, ReportsState>(
              builder: (context, state) {
                if (state is BookingSourceReportLoading ||
                    state is ReportsInitial) {
                  return _ShimmerSkeleton(colors: colors);
                }
                if (state is BookingSourceReportError) {
                  return Center(
                    child: Padding(
                ...
                if (state is BookingSourceReportLoaded) {
